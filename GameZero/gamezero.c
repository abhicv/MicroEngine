...
void ShowMainMenu(u32 *gameMode, SDL_Event *event, SDL_Renderer *renderer)
{
    ...
    while(!quitMainMenu)
    {
        ...
        MUI_TextEdit(&ui, GEN_MUI_ID(), rect, buttonStyle, &textEdit);
...
void GameUpdateAndRender(u32 *gameMode, bool *quitGame, f32 deltaTime, SDL_Renderer *renderer)
{
    MUI_BeginFrame(&ui, &uiInput);
    {
        ...
        textRect = ME_RectInit(120, 20, 120, 300);
        ...
        textRect = ME_RectInit(800, 20, 120, 20);
        playerHealth = MUI_Slider(&ui, GEN_MUI_ID(), playerHealth, textRect, sliderStyle);
    }

    PhysicsSystem(&ecsWorld, gravity, 0.03f);
    ...
    for(u32 i = 0; i < ecsWorld.activeEntityCount; i++)
    {
        if (!IsEntityDead(i, &ecsWorld))
        {
            ...
            switch (ecsWorld.tags[i])
            {
                ...
                {
                    //printf("normal: {%f, %f}\n", ecsWorld.physics[i].physicsBody.collisionNormal.x, ecsWorld.physics[i].physicsBody.collisionNormal.y);
                    ...
                    masked = ecsWorld.physics[i].tagOfCollidedEntity & (ENTITY_TAG_LIZARD | ENTITY_TAG_SLIME | ENTITY_TAG_FLYEE | ENTITY_TAG_ENEMY_BULLET);

                    if (masked == ENTITY_TAG_LIZARD ||
                        masked == ENTITY_TAG_SLIME ||
                        masked == ENTITY_TAG_FLYEE ||
                        masked == ENTITY_TAG_ENEMY_BULLET)
                    {
