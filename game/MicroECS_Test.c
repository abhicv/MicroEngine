...
global MicroECSWorld ecsWorld = {0};
global MUI ui = {0};
...
void Update(float deltaTime)
{
    ...
    MUI_TextP(&ui, GEN_MUI_ID(), textRect, entityCountString, 16);
        ...
        if (!IsEntityDead(i, &ecsWorld))
        {
            ...
            case ENTITY_TAG_lizard:
                if (MECS_EntitySignatureEquals(ecsWorld.entitySignature[i], TRANSFORM_COMPONENT_SIGN | ANIMATION_COMPONENT_SIGN | ENTITYSTAT_COMPONENT_SIGN))
                {
                    ...
                    MUI_Rect r = {pos.x, pos.y - 60, 80, 10};
                    health = MUI_SliderP(&ui, GEN_MUI_IDi(i), health, r);
                }
                break;
...
int main(int argc, char *argv[])
{
    ...
    tileMap = ME_LoadTileMap("test.tmap");
    ...
    loop(m, 10)
    {
        ...
        if (ENTITY_INDEX_VALID(platform))
        {
            ...
            transform.position = Vector2Init(120.0f * (f32)m, 700);
            render.width = 100;
            render.height = 20;
            render.texture = NULL;
    ...
    loop(n, 5)
    {
        ...
        lizardTransform.position.y = ME_Random(100, 600);
