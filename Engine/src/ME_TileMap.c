...
TileMap ME_LoadTileMap(const char *fileName)
{
    ...
    if(tileMapFile != NULL)
    {
        ...
        while(c != EOF)
        {
            ...
            {
            case 'w':
                fscanf(tileMapFile, ":%d", &tileMapWidth);
                // printf("w:%d\n", tileMapWidth);
                break;

            case 'h':
                fscanf(tileMapFile, ":%d", &tileMapHeight);
                // printf("h:%d\n", tileMapHeight);
                break;

            case 't':
                char a = fgetc(tileMapFile);
                if (a == 'w')
                {
                    fscanf(tileMapFile, ":%d", &tileWidth);
                    // printf("tw:%d\n", tileWidth);
                }
                else if (a == 'h')
                {
                    fscanf(tileMapFile, ":%d", &tileHeight);
                    // printf("th:%d\n", tileHeight);
                }
                else if (a == ':') //tile coords
                {
                    ...
                    fscanf(tileMapFile, "%d,%d,%d,%d", &tileMapPosX,
                                                    &tileMapPosY,
                                                    &tileSheetPosX,
                                                    &tileSheetPosY);
            ...
            case 's':
                char b = fgetc(tileMapFile);
                if (b == 'w')
                {
                    fscanf(tileMapFile, ":%d", &tileSheetTileWidth);
                    // printf("sw:%d\n", tileSheetTileWidth);
                }
                else if (b == 'h')
                {
                    fscanf(tileMapFile, ":%d", &tileSheetTileHeight);
                    // printf("sh:%d\n", tileSheetTileHeight);
